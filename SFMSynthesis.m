...
Z = data(1,2); % number of links
nStages = data(1,3); % number of stages
C = data(1,4); % control cycle time-step (s)
c = data(1,5); % factor c - threshold for definition of a jam
Tsim = data(1,6); % simulation time-step (s)
...
save(sprintf("%s/%s.mat",folder,folder),'junctionTable','linksTable','S','turningRatesTable','t0','T',...
    'J','Z','nStages','C','c','Tsim','lostTime','nStagesJunction','gmin','capacity','saturation',...
    'lanes','x0','junctions','links','inLinks','notInLinks','A','Bu','BG','Bg','Bu_sim',...
    'BG_sim','Bg_sim','C_z','E_DTUC_psi','E_DTUC_phi','E_D2TUC_psi','E_D2TUC_phi');
end
