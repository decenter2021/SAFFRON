...
d_model_A = (rand(size(model.d))*0.25+0.25).*model.d; % Amplitude is 0-0.25 of nominal demand
...
d_model_spike_links = [7 20 22];%round((model.Z-1)*rand(1,d_model_spike_N_links))+1;
d_model_spike_mag_of_d = [5 15 30];
d_model_spike_t = ones(3,1)*(rand(1,1)+1.5)*60*60;%(rand(d_model_spike_N_links,1)+1.5)*60*60;
...
for i = [12 33 39 d_model_spike_links]
    plot(tspan/(60*60),d(i,:)*(60*60),'LineWidth',3);   
    %plot(tspan(1:end-1)/(60*60),dNL{1,1}(i,:),'--k','LineWidth',1);
end
for i = [12 33 39 d_model_spike_links]
    plot(tspan/(60*60),(60*60)*model.d(i)*ones(size(tspan/(60*60))),'--k','LineWidth',2);
end
legend({'$z=12$','$z=33$','$z=39$',sprintf("$z=%d$",d_model_spike_links(1)),sprintf("$z=%d$",d_model_spike_links(2)),sprintf("$z=%d$",d_model_spike_links(3)),'$e_z^{\mathrm{\,hist}}$'}, 'Interpreter','latex');
ylabel("$e_z \quad [\mathrm{veh}\;\mathrm{h}^{-1}]$",'Interpreter','latex');
xlabel('hours','Interpreter','latex');
% set(gcf,'renderer','Painters');
% exportgraphics(gcf,'./figures/d_sc1.pdf');
hold off;
...
xlabel('hours','Interpreter','latex');
% set(gcf,'renderer','Painters');
% exportgraphics(gcf,'./figures/x_hat_sc1.pdf');
hold off;
...
set(gca,'TickLabelInterpreter','latex');
for i = link %d_model_spike_links(1) %[15 40 38 41 16 17]
    plot(tspan(1:T_est_Tsim:end)/(60*60),(60*60)*dhat{m,1}(i,1:length(1:T_est_Tsim:end)),'LineWidth',3);
    %plot(tspan/(60*60),dhat{m,1}(i,:),'LineWidth',3);
    plot(tspan(1:end-1)/(60*60),(60*60)*dNL{m,1}(i,:),'LineWidth',3);
end
legend({'$\hat{e}_z$','$e_z$'}, 'Interpreter','latex');
ylabel(sprintf("$e_z \\quad [\\mathrm{veh}\\;\\mathrm{h}^{-1}] \\quad (z = %d)$",link),'Interpreter','latex');
xlabel('hours','Interpreter','latex');
% set(gcf,'renderer','Painters');
% exportgraphics(gcf,'./figures/d_hat_sc1.pdf');
hold off;
...
xlabel('hours','Interpreter','latex');
% set(gcf,'renderer','Painters');
% exportgraphics(gcf,'./figures/sum_x_sc1.pdf');
hold off;
...
xlabel('hours','Interpreter','latex');
% set(gcf,'renderer','Painters');
% exportgraphics(gcf,'./figures/g3_sc1.pdf');
hold off;
end

%% Green-times junction 7
figure('Position',3*[0 0 192 144]);
hold on;
grid on;
box on;
...
xlabel('hours','Interpreter','latex');
% set(gcf,'renderer','Painters');
% exportgraphics(gcf,'./figures/g7_sc1.pdf');
hold off;
